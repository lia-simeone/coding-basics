...
def not_bad(s):
  ...
  not_pos=s.find('not')
  bad_pos=s.find('bad')
  if bad_pos>not_pos and bad_pos>0 and not_pos>0:
    result=s.replace(s[not_pos:bad_pos+3],'good')
  else:
    result=s
  return result
...
def front_back(a, b):
  ...
  #join concatenates these two lists with the '' delimiter. it's a method of string.
